...
class FreqNet(nn.Module):
    def __init__(self, in_chans = 1, base_channels = 64):
        ...
        # prob layers
        self.prob_conv = nn.Conv1d(base_channels, in_chans, 1)  # Probability matrix branch
        self.sigmoid = nn.Sigmoid()  # For probability values [0,1]
    ...
    def forward(self, x):
        ...
        d1 = self.decoder1(b) + e6
        d2 = self.up1(d1) + e4

        d3 = self.decoder2(d2) + e3
        d4 = self.up2(d3) + e1
        ...
        prob = self.sigmoid(self.prob_conv(d5))  # Probability matrix
